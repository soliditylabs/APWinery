...
interface IController {
    ...
    event FutureRegistered(address _futureVault);
    event FutureUnregistered(address _futureVault);
    event StartingDelaySet(uint256 _startingDelay);
    event NewPerformanceFeeFactor(address _futureVault, uint256 _feeFactor);
    event FutureTerminated(address _futureVault);
    event DepositsPaused(address _futureVault);
    event DepositsResumed(address _futureVault);
    event WithdrawalsPaused(address _futureVault);
    event WithdrawalsResumed(address _futureVault);
    event RegistryChanged(address _registry);
    event FutureSetToBeTerminated(address _futureVault);
     ...
     * @return the list of futureVault durations
     */
